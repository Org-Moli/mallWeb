...
import com.weixin.service.accesstoken.impl.AccesstokenService;
...
public class Accesstoken {
	...
	public static String getaccesstoken() throws Exception {
		...
		AccesstokenService accesstokenService = (AccesstokenService) webctx.getBean("accesstokenService");
		String accesstoken = "";
		pd.put("accesstoken_id", "1");
		PageData  token  =  accesstokenService.findById(pd);;
		Long add_time = new Long(token.getString("add_time"));
	//	Token token = new Token();
	//如果数据库保存得时间加上7200秒（2小时）<=当前时间，说明token已经过期，需重新获取，并更新到数据库
		if(add_time+(7200*1000) <= nowtime){ 
		PageData config = Config.getconfig();
		String token_url = Config.token_url2.replace("APPID",config.getString("appid")).replace("APPSECRET",config.getString("appsecret"));
