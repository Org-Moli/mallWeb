...
public class UsercouponController extends BaseController {
	...
	public ModelAndView list(Page page) throws Exception{
		...
		mv.setViewName("usercoupon/usercoupon_list");
	...
	public ModelAndView exportExcel() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"导出Usercoupon到excel");
		...
		List<PageData> varOList = usercouponService.listAll(pd);
		List<PageData> varList = new ArrayList<PageData>();
		for(int i=0;i<varOList.size();i++){
			PageData vpd = new PageData();
			vpd.put("var1", varOList.get(i).getString("coupon_id"));	//1
			vpd.put("var2", varOList.get(i).get("status").toString());	//2
			varList.add(vpd);
		}
