    ...
    public static void modifyJobTime(String jobName, String time) {  
        try {  
           ...
           // TriggerKey triggerKey = TriggerKey.triggerKey(jobName,TRIGGER_GROUP_NAME); 		//通过触发器名和组名获取TriggerKey
            CronTrigger trigger = (CronTrigger)sched.getTrigger(jobName,JOB_GROUP_NAME);				//通过TriggerKey获取CronTrigger
            if (trigger == null) {  
                return;  
            }  
    ...
    public static void modifyJobTime(String triggerName, String triggerGroupName, String time) {  
        try {  
           ...
           // TriggerKey triggerKey = TriggerKey.triggerKey(triggerName,triggerGroupName); 	//通过触发器名和组名获取TriggerKey
            CronTrigger trigger = (CronTrigger)sched.getTrigger(triggerName,JOB_GROUP_NAME);//通过TriggerKey获取CronTrigger
            if (trigger == null)  return;  
            CronScheduleBuilder scheduleBuilder = CronScheduleBuilder.cronSchedule(trigger.getCronExpression());
            String oldTime = trigger.getCronExpression();  
            if (!oldTime.equalsIgnoreCase(time)) {  
//            	trigger = 
//            			(CronTrigger)trigger.getTriggerBuilder()		//重新构建trigger
//            			.withIdentity(triggerName,triggerGroupName)
...
//		                 .withSchedule(scheduleBuilder).build();
		            	 sched.rescheduleJob(triggerName,triggerGroupName, trigger);				//按新的trigger重新设置job执行
            }
        } catch (Exception e) {  
            throw new RuntimeException(e);  
        }  
    ...
    public static void removeJob(String jobName, String jobGroupName,String triggerName, String triggerGroupName) {  
        try {  
          ...
          //  JobKey jobKey = JobKey.jobKey(jobName, jobGroupName);							//通过任务名和组名获取JobKey
            sched.pauseTrigger(triggerName,triggerGroupName); //(triggerKey);	// 停止触发器  
